...
pub struct HipblasLtHandle(pub *mut c_void);
#[repr(transparent)]
#[derive(Copy, Clone, Debug, Default)]
pub struct HipblasLtMatmulDesc(pub *mut c_void);
#[repr(transparent)]
#[derive(Copy, Clone, Debug, Default)]
pub struct HipblasLtMatrixLayout(pub *mut c_void);
#[repr(transparent)]
#[derive(Copy, Clone, Debug, Default)]
pub struct HipblasLtMatmulPreference(pub *mut c_void);

unsafe impl Send for HipblasLtHandle {}
unsafe impl Sync for HipblasLtHandle {}
unsafe impl Send for HipblasLtMatmulDesc {}
unsafe impl Sync for HipblasLtMatmulDesc {}
unsafe impl Send for HipblasLtMatrixLayout {}
unsafe impl Sync for HipblasLtMatrixLayout {}
unsafe impl Send for HipblasLtMatmulPreference {}
unsafe impl Sync for HipblasLtMatmulPreference {}

pub use crate::hipblas::{HipblasDataType, HipblasOp, HipblasStatus};
...
#[repr(C)]
#[derive(Copy, Clone, Debug, Default)]
pub struct HipblasLtMatmulAlgo {
    pub data: [u64; 8],
}
...
pub struct HipblasLtFns {
    ...
    pub hipblasLtMatmulDescCreate: unsafe extern "C" fn(
        *mut HipblasLtMatmulDesc,
        HipblasComputeType,
        HipblasDataType,
    ) -> HipblasStatus,
    pub hipblasLtMatmulDescDestroy: unsafe extern "C" fn(HipblasLtMatmulDesc) -> HipblasStatus,
    pub hipblasLtMatmulDescSetAttribute: unsafe extern "C" fn(
        HipblasLtMatmulDesc,
        HipblasLtMatmulDescAttr,
        *const c_void,
        usize,
    ) -> HipblasStatus,

    pub hipblasLtMatrixLayoutCreate: unsafe extern "C" fn(
        *mut HipblasLtMatrixLayout,
        HipblasDataType,
        u64,
        u64,
        i64,
    ...
    pub hipblasLtMatrixLayoutSetAttribute: unsafe extern "C" fn(
        HipblasLtMatrixLayout,
        HipblasLtMatrixLayoutAttr,
        *const c_void,
        usize,
    ) -> HipblasStatus,

    pub hipblasLtMatmulPreferenceCreate:
        unsafe extern "C" fn(*mut HipblasLtMatmulPreference) -> HipblasStatus,
    pub hipblasLtMatmulPreferenceDestroy:
        unsafe extern "C" fn(HipblasLtMatmulPreference) -> HipblasStatus,
    pub hipblasLtMatmulPreferenceSetAttribute: unsafe extern "C" fn(
        HipblasLtMatmulPreference,
        HipblasLtMatmulPreferenceAttr,
        *const c_void,
        usize,
    ) -> HipblasStatus,
...
static LIB: LazyLock<LoaderResult<Library>> =
    LazyLock::new(|| try_load(&["libhipblaslt.so", "libhipblaslt.so.0", "hipblaslt.dll"]));
static FNS: OnceLock<LoaderResult<HipblasLtFns>> = OnceLock::new();

fn load_fns(lib: &Library) -> LoaderResult<HipblasLtFns> {
    macro_rules! g {
        ($s:ident) => {
            sym(lib, "hipblaslt", stringify!($s))?
        };
    }
    unsafe {
        Ok(HipblasLtFns {
