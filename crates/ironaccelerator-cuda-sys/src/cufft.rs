...
pub enum CufftResult {
    Success = 0, InvalidPlan = 1, AllocFailed = 2, InvalidType = 3,
    InvalidValue = 4, InternalError = 5, ExecFailed = 6, SetupFailed = 7,
    InvalidSize = 8, UnalignedData = 9, IncompleteParameterList = 10,
    InvalidDevice = 11, ParseError = 12, NoWorkspace = 13, NotImplemented = 14,
    LicenseError = 15, NotSupported = 16, Other = 0xFFFF_FFFF,
}

impl CufftResult {
    pub fn from_raw(r: u32) -> Self {
        if r <= 16 { unsafe { std::mem::transmute(r) } } else { Self::Other }
    }
...
pub enum CufftType {
    R2C = 0x2a, C2R = 0x2c, C2C = 0x29,
    D2Z = 0x6a, Z2D = 0x6c, Z2Z = 0x69,
}
...
pub struct CufftFns {
    ...
    pub cufftPlan1d: unsafe extern "C" fn(
        *mut CufftHandle, c_int, CufftType, c_int,
    ) -> CufftResult,
    pub cufftPlanMany: unsafe extern "C" fn(
        *mut CufftHandle, c_int, *const c_int,
        *const c_int, c_int, c_int,
        *const c_int, c_int, c_int,
        CufftType, c_int,
    ) -> CufftResult,
    ...
    pub cufftExecC2C: unsafe extern "C" fn(CufftHandle, CUdeviceptr, CUdeviceptr, c_int) -> CufftResult,
    pub cufftExecZ2Z: unsafe extern "C" fn(CufftHandle, CUdeviceptr, CUdeviceptr, c_int) -> CufftResult,
}

fn candidates() -> &'static [&'static str] {
    &["libcufft.so.11", "libcufft.so.10", "libcufft.so",
      "cufft64_11.dll", "cufft64_10.dll"]
}
...
pub fn fns() -> Result<&'static CufftFns, &'static LoadError> { FNS.as_ref() }
