...
use ironaccelerator_core::{BackendKind, ComputeTier, DType, WorkloadKind};
...
pub struct FilterSpec {
    ...
    /// Only keep strategies whose `supported_dtypes` contains this dtype.
    /// Strategies with an empty dtype list are treated as "any" and pass.
    pub dtype: Option<DType>,
    /// Only keep strategies whose `supported_phases` contains this phase.
    /// Strategies with an empty phase list are treated as "any" and pass.
    pub phase: Option<Phase>,
    /// Only keep hardware (when `hardware_id` is unset, filters the set of
    /// edge-weighted candidates) whose compute tier is at least this level.
    pub min_compute_tier: Option<ComputeTier>,
}
...
impl Ontology {
    ...
    pub fn query(&self, filter: &FilterSpec, rank_by: RankBy) -> Vec<Id> {
        let mut hits: Vec<(&StrategyClass, f32)> =
            self.strategies
                .values()
                .filter(|s| {
                    if filter.forbid_jit && s.jit {
                        return false;
                    }
                    if filter.require_open_source && !s.open_source {
                        return false;
                    }
                    if let Some(b) = filter.backend {
                        if !s.backends.contains(&b) {
                            return false;
                        }
                    }
                    ...
                    if let Some(dt) = filter.dtype {
                        if !s.supported_dtypes.is_empty() && !s.supported_dtypes.contains(&dt) {
                            return false;
                        }
                    }
                    if let Some(ph) = filter.phase {
                        if !s.supported_phases.is_empty() && !s.supported_phases.contains(&ph) {
                            return false;
                        }
                    }
                    if let Some(min_tier) = filter.min_compute_tier {
                        let tier_hw_id = filter
                            .hardware_id
                            .as_ref()
                            .and_then(|h| self.hardware.get(&Id(h.clone())));
                        if let Some(hw) = tier_hw_id {
                            if hw.compute_tier < min_tier {
                                return false;
                            }
                        } else {
                            let ok = self.hardware.values().any(|h| {
                                s.backends.contains(&h.backend) && h.compute_tier >= min_tier
                            });
                            if !ok {
                                return false;
                            }
    ...
    pub fn explain(&self, strategy: &str, hardware: Option<&str>) -> Option<Explanation> {
        ...
        let workloads: Vec<WorkloadClass> = s
            .workloads
            .iter()
            .filter_map(|w| self.workloads.get(w).cloned())
            .collect();
        let optimizations: Vec<Optimization> = s
            .optimizations
            .iter()
            .filter_map(|o| self.optimizations.get(o).cloned())
            .collect();
        let matched_hardware = hardware.and_then(|h| self.hardware.get(&Id(h.into())).cloned());
        ...
        if s.jit {
            n.push_str("Note: requires runtime JIT compilation.\n");
        }
        if !s.open_source {
            n.push_str("Note: closed-source vendor library.\n");
        }
...
mod tests {
    ...
    fn explain_produces_narrative() {
        let o = Ontology::global();
        let ex = o
            .explain("strategy.flashattn.v3", Some("hardware.nvidia.h100"))
            .unwrap();
    ...
    #[test]
    fn dtype_filter_rules_out_fp8_strategies_for_fp16_query() {
        let o = Ontology::global();
        let ids = o.query(
            &FilterSpec {
                backend: Some(BackendKind::Cuda),
        ...
        // cuBLASLt BF16/F16 path should remain; FP8-only TE recipe should not.
        assert!(ids.iter().any(|i| i.0 == "strategy.cublaslt.bf16"));
        assert!(!ids.iter().any(|i| i.0 == "strategy.cublaslt.fp8_te"));
    }

    #[test]
    fn min_tier_filter_rejects_consumer_only_when_datacenter_required() {
        let o = Ontology::global();
        let ids = o.query(
            &FilterSpec {
                backend: Some(BackendKind::Cuda),
                workload: Some(WorkloadKind::Gemm),
                hardware_id: Some("hardware.nvidia.rtx3090ti".into()),
                min_compute_tier: Some(ComputeTier::Datacenter),
                ..Default::default()
            },
            RankBy::Name,
        );
        assert!(
            ids.is_empty(),
            "consumer GPU should be rejected by Datacenter tier floor"
        );
    }
    ...
    fn ranking_prefers_high_edge_weight() {
        ...
        assert_eq!(
            ids.first().map(|i| i.0.as_str()),
            Some("strategy.flashattn.v3")
        );
    }
}
