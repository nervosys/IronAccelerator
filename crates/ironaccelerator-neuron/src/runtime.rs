...
impl Model {
    ...
    pub fn load(neff: &[u8], start_nc: i32, nc_count: i32) -> Result<Self, NeuronError> {
        ...
        if r != NRT_SUCCESS { return Err(NeuronError::Runtime(r)); }
        Ok(Self { l, handle })
    }
    ...
    pub fn execute(
        ...
        let exec = self.l.nrt_execute.ok_or(NeuronError::MissingSymbol("nrt_execute"))?;
        let r = unsafe { exec(self.handle, inputs.handle, outputs.handle) };
        if r != NRT_SUCCESS { return Err(NeuronError::Runtime(r)); }
        Ok(())
    }

    pub fn raw(&self) -> NrtModelHandle { self.handle }
}

impl Drop for Model {
    fn drop(&mut self) {
        if let Some(unload) = self.l.nrt_unload {
            unsafe { let _ = unload(self.handle); }
        }
    }
}
...
impl TensorSet {
    ...
    pub fn add(&mut self, name: &CStr, tensor: *mut c_void) -> Result<(), NeuronError> {
        let add = self
            .l
    ...
    pub fn raw(&self) -> NrtTensorSetHandle { self.handle }
}

impl Drop for TensorSet {
    fn drop(&mut self) {
        if let Some(free) = self.l.nrt_free_tensor_set {
            unsafe { let _ = free(self.handle); }
        }
    }
}
