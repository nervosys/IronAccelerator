...
impl MetalDType {
    ...
    fn bytes(self) -> u64 {
        match self { Self::F32 => 4, Self::F16 => 2 }
    }
}
...
pub fn gemm(
    ...
    {
        return Err(Error::InvalidArgument("gemm: buffer too small for declared shape"));
    }

    let desc_a = MatrixDescriptor::init_single(
        a_rows as u64, a_cols as u64, row_bytes(a_cols), dt);
    let desc_b = MatrixDescriptor::init_single(
        b_rows as u64, b_cols as u64, row_bytes(b_cols), dt);
    let desc_c = MatrixDescriptor::init_single(
        c_rows as u64, c_cols as u64, row_bytes(c_cols), dt);
...
unsafe impl Send for GemmPlan {} unsafe impl Sync for GemmPlan {}
...
impl GemmPlan {
    ...
    pub fn new(
        ...
        let mm = MatrixMultiplication::init(
            device.raw(), trans_a, trans_b,
            m as u64, n as u64, k as u64,
            alpha, beta,
        ).ok_or(Error::Other("Metal: MPSMatrixMultiplication init failed"))?;
        Ok(Self { mm, _device: device })
    }
