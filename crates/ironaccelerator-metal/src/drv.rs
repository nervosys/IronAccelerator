...
use metal::{
    CommandQueue, Device as MtlDevice, MTLResourceOptions,
    Buffer as MtlBuffer,
};
use std::sync::Arc;
...
impl Device {
    ...
    #[inline] pub fn name(&self) -> String { self.inner.name().to_string() }
    #[inline] pub fn registry_id(&self) -> u64 { self.inner.registry_id() }
    ...
    pub fn apple_family(&self) -> u32 {
        ...
        for (family, n) in [
            ...
            (Apple5, 5), (Apple4, 4), (Apple3, 3),
        ] {
            if self.inner.supports_family(family) { return n; }
        }
        0
    }

    #[inline] pub fn has_unified_memory(&self) -> bool { self.inner.has_unified_memory() }
    #[inline] pub fn max_buffer_length(&self) -> u64 { self.inner.max_buffer_length() as u64 }
    #[inline] pub fn recommended_max_working_set_size(&self) -> u64 {
        self.inner.recommended_max_working_set_size()
    }

    #[inline] pub fn raw(&self) -> &MtlDevice { &self.inner }

    pub fn new_queue(self: &Arc<Self>) -> Arc<Queue> {
        let q = self.inner.new_command_queue();
        Arc::new(Queue { device: self.clone(), inner: q })
    }
    ...
    pub fn new_buffer(self: &Arc<Self>, bytes: usize, shared: bool) -> Result<Arc<Buffer>> {
        ...
        let opts = if shared { MTLResourceOptions::StorageModeShared }
                   else      { MTLResourceOptions::StorageModePrivate };
        let buf = self.inner.new_buffer(bytes as u64, opts);
        Ok(Arc::new(Buffer { _device: self.clone(), inner: buf, bytes }))
    }
}
...
unsafe impl Send for Queue {} unsafe impl Sync for Queue {}

impl Queue {
    #[inline] pub fn device(&self) -> &Arc<Device> { &self.device }
    #[inline] pub fn raw(&self) -> &CommandQueue { &self.inner }
...
unsafe impl Send for Buffer {} unsafe impl Sync for Buffer {}

impl Buffer {
    #[inline] pub fn raw(&self) -> &MtlBuffer { &self.inner }
    #[inline] pub fn bytes(&self) -> usize { self.bytes }

    /// Host-visible pointer for `StorageModeShared` buffers. `None` on
    /// private-storage buffers.
    pub fn contents(&self) -> *mut std::ffi::c_void { self.inner.contents() }
}
