//! NCCL safe wrapper.
//!
//! Single-process multi-GPU groups use [`CommGroup::from_devices`], which
...
fn dtype_of<T: Repr>() -> Result<sys::NcclDataType> {
    let t = TypeId::of::<T>();
    Ok(if t == TypeId::of::<i8>() {
        sys::NcclDataType::Int8
    } else if t == TypeId::of::<u8>() {
        sys::NcclDataType::Uint8
    } else if t == TypeId::of::<i32>() {
        sys::NcclDataType::Int32
    } else if t == TypeId::of::<u32>() {
        sys::NcclDataType::Uint32
    } else if t == TypeId::of::<i64>() {
        sys::NcclDataType::Int64
    } else if t == TypeId::of::<u64>() {
        sys::NcclDataType::Uint64
    } else if t == TypeId::of::<f32>() {
        sys::NcclDataType::Float32
    } else if t == TypeId::of::<f64>() {
        sys::NcclDataType::Float64
    } else {
        return Err(Error::Other(
            "nccl: unsupported element type (use i8/u8/i32/u32/i64/u64/f32/f64)",
        ));
    })
}
...
impl Drop for RankComm {
    fn drop(&mut self) {
        if let Ok(f) = fns() {
            unsafe {
                let _ = (f.ncclCommDestroy)(self.comm);
            }
        }
    }
}
...
impl CommGroup {
    ...
    pub fn new_id() -> Result<Id> {
        ...
        unsafe {
            check("ncclGetUniqueId", (f.ncclGetUniqueId)(&mut id))?;
        }
        Ok(id)
    }

    /// Single-process multi-GPU init. Creates one communicator per device,
    /// numbered in argument order.
    pub fn from_devices(devices: &[Arc<Device>]) -> Result<Self> {
        let f = fns()?;
        let n = devices.len() as c_int;
        if n == 0 {
            return Err(Error::Other("nccl::from_devices: empty device list"));
        }
        let id = Self::new_id()?;
        let mut comms: Vec<RankComm> = Vec::with_capacity(devices.len());
        unsafe {
            check("ncclGroupStart", (f.ncclGroupStart)())?;
        }
        ...
        unsafe {
            check("ncclGroupEnd", (f.ncclGroupEnd)())?;
        }
        Ok(Self { comms })
    }

    /// Multi-process init: one rank per process, using a shared `id`.
    pub fn from_rank(device: &Arc<Device>, rank: usize, world_size: usize, id: Id) -> Result<Self> {
        let f = fns()?;
        device.bind()?;
        let mut c = sys::NcclComm::default();
        unsafe {
    ...
    fn pick(&self, rank: usize) -> Result<&RankComm> {
        self.comms
            .get(rank)
            .ok_or(Error::Other("nccl: rank out of range"))
    }
    ...
    pub fn all_reduce<T: Repr>(
        ...
        unsafe {
            check(
                "ncclAllReduce",
                (f.ncclAllReduce)(
                    send.device_ptr() as *const _,
                    recv.device_ptr() as *mut _,
                    send.len(),
                    dtype_of::<T>()?,
                    op.to_sys(),
                    rc.comm,
    ...
    pub fn all_reduce_in_place<T: Repr>(
        ...
        unsafe {
            check(
                "ncclAllReduce",
                (f.ncclAllReduce)(
                    buf.device_ptr() as *const _,
                    buf.device_ptr() as *mut _,
                    buf.len(),
                    dtype_of::<T>()?,
                    op.to_sys(),
                    rc.comm,
    ...
    pub fn group<F: FnOnce() -> Result<()>>(f: F) -> Result<()> {
        let n = fns()?;
        unsafe {
            check("ncclGroupStart", (n.ncclGroupStart)())?;
        }
        let r = f();
        unsafe {
            check("ncclGroupEnd", (n.ncclGroupEnd)())?;
        }
        r
    }
}

pub fn world_from_all_devices() -> Result<(Vec<Arc<Device>>, CommGroup)> {
    let n = Device::count()?;
    let devices: Vec<Arc<Device>> = (0..n)
        .map(Device::open)
        .collect::<crate::drv::Result<Vec<_>>>()
        .map_err(|e| Error::Backend {
            backend: ironaccelerator_core::BackendKind::Cuda,
            code: e.numeric(),
        })?;
    let group = CommGroup::from_devices(&devices)?;
    Ok((devices, group))
}
