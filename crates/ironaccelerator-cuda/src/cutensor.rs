...
pub use sys::{
    CutensorAlgo as Algo, CutensorComputeDesc as ComputeDesc, CutensorJitMode as JitMode,
    CutensorOperator as Operator, CutensorStatus, CutensorWorksizePref as WorksizePref,
};
...
impl CutensorHandle {
    pub fn new(device: Arc<drv::Device>) -> Result<Arc<Self>> {
        ...
        unsafe {
            check("cutensorCreate", (f.cutensorCreate)(&mut h))?;
        }
        Ok(Arc::new(Self {
            handle: h,
            _device: device,
    ...
    #[inline]
    pub fn raw(&self) -> sys::CutensorHandle {
        self.handle
    }
}
...
impl Drop for CutensorHandle {
    fn drop(&mut self) {
        if let Ok(f) = fns() {
            unsafe {
                let _ = (f.cutensorDestroy)(self.handle);
            }
        }
    }
}
...
fn dtype_of<T: Repr>() -> Result<CudaDataType> {
    ...
    Ok(if t == TypeId::of::<f32>() {
        ...
        return Err(Error::Other(
            "cutensor: unsupported element type (use f32/f64)",
        ));
    })
}
...
unsafe impl Send for TensorDescr {}
unsafe impl Sync for TensorDescr {}
...
impl TensorDescr {
    ...
    #[inline]
    pub fn raw(&self) -> sys::CutensorTensorDescr {
        self.raw
    }
}
...
impl Drop for TensorDescr {
    fn drop(&mut self) {
        if let Ok(f) = fns() {
            unsafe {
                let _ = (f.cutensorDestroyTensorDescriptor)(self.raw);
            }
        }
    }
}
    ...
    pub fn build(
        ...
        unsafe {
            check(
                ...
                (f.cutensorCreatePlanPreference)(handle.raw(), &mut pref, Algo::Default, jit),
            )?;
        }
