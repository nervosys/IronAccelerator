...
use iron_qnn_sys::qnn::{
    ...
    Qnn_GraphHandle_t, Qnn_ErrorHandle_t, QNN_SUCCESS, Target,
};
...
#[derive(Debug, Clone)]
pub enum Error {
    NotAvailable { target: Target, detail: String },
    Call { op: &'static str, code: Qnn_ErrorHandle_t },
...
impl From<&LoadError> for Error {
    fn from(e: &LoadError) -> Self {
        Error::NotAvailable { target: Target::Htp, detail: format!("{e}") }
    }
}
...
fn iface(target: Target) -> Result<&'static QnnInterfaceV2> {
    sys::fns(target)
        .map(|f| f.iface)
        .map_err(|e| Error::NotAvailable { target, detail: format!("{e}") })
}

#[inline]
fn check(op: &'static str, code: Qnn_ErrorHandle_t) -> Result<()> {
    if code == QNN_SUCCESS { Ok(()) } else { Err(Error::Call { op, code }) }
}
...
unsafe impl Send for Backend {} unsafe impl Sync for Backend {}
...
impl Backend {
    pub fn new(target: Target) -> Result<Arc<Self>> {
        ...
        unsafe {
            check("QnnBackend_create",
                  (i.backend_create)(std::ptr::null_mut(), std::ptr::null(), &mut h))?;
        }
        Ok(Arc::new(Self { target, handle: h }))
    }

    #[inline] pub fn target(&self) -> Target { self.target }
    #[inline] pub fn raw(&self) -> Qnn_BackendHandle_t { self.handle }

    pub fn api_version(&self) -> Result<sys::QnnApiVersion> {
        let i = iface(self.target)?;
        let mut v = sys::QnnApiVersion {
            core_major: 0, core_minor: 0, core_patch: 0,
            backend_major: 0, backend_minor: 0, backend_patch: 0,
        };
        unsafe { check("QnnBackend_getApiVersion", (i.backend_getApiVersion)(&mut v))?; }
        Ok(v)
    }

    pub fn provider_name(&self) -> Result<String> {
        Ok(sys::fns(self.target)
            .map_err(|e| Error::NotAvailable { target: self.target, detail: format!("{e}") })?
            .provider_name.clone())
    }
}
...
impl Drop for Backend {
    fn drop(&mut self) {
        if let Ok(i) = iface(self.target) {
            unsafe { let _ = (i.backend_free)(self.handle); }
        }
    }
}
...
impl Device {
    pub fn new(backend: Arc<Backend>) -> Result<Arc<Self>> {
        ...
        unsafe {
            check("QnnDevice_create",
                  (i.device_create)(std::ptr::null_mut(), std::ptr::null(), &mut h))?;
        }
        Ok(Arc::new(Self { backend, handle: h }))
    }

    #[inline] pub fn backend(&self) -> &Arc<Backend> { &self.backend }
    #[inline] pub fn raw(&self) -> Qnn_DeviceHandle_t { self.handle }
}

impl Drop for Device {
    fn drop(&mut self) {
        if let Ok(i) = iface(self.backend.target) {
            unsafe { let _ = (i.device_free)(self.handle); }
        }
    }
}
...
unsafe impl Send for Context {} unsafe impl Sync for Context {}
...
impl Context {
    pub fn new(device: Arc<Device>) -> Result<Arc<Self>> {
        ...
        unsafe {
            check("QnnContext_create",
                  (i.context_create)(device.backend.handle, device.handle,
                                     std::ptr::null(), &mut h))?;
        }
    ...
    pub fn from_binary(device: Arc<Device>, blob: &[u8]) -> Result<Arc<Self>> {
        ...
        let mut h: Qnn_ContextHandle_t = std::ptr::null_mut();
        unsafe {
            check("QnnContext_createFromBinary",
                  (i.context_createFromBinary)(
                      device.backend.handle, device.handle,
                      std::ptr::null(),
                      blob.as_ptr() as *const c_void, blob.len(),
                      &mut h, std::ptr::null_mut()))?;
        }
        Ok(Arc::new(Self { device, handle: h }))
    }
    ...
    pub fn to_binary(&self) -> Result<Vec<u8>> {
        ...
        unsafe { check("QnnContext_getBinarySize",
                       (i.context_getBinarySize)(self.handle, &mut size))?; }
        let mut buf = vec![0u8; size];
        let mut written: usize = 0;
        unsafe {
            check("QnnContext_getBinary",
                  (i.context_getBinary)(self.handle,
    ...
    #[inline] pub fn raw(&self) -> Qnn_ContextHandle_t { self.handle }
}

impl Drop for Context {
    fn drop(&mut self) {
        if let Ok(i) = iface(self.device.backend.target) {
            unsafe { let _ = (i.context_free)(self.handle, std::ptr::null_mut()); }
        }
    }
}
...
unsafe impl Send for Graph {} unsafe impl Sync for Graph {}
...
impl Graph {
    ...
    pub unsafe fn add_node_raw(&mut self, op_config: *const sys::QnnOpConfig) -> Result<()> {
        if self.finalized {
            return Err(Error::Precondition {
                op: "Graph::add_node_raw", msg: "graph already finalized".into(),
            });
        }
        let i = iface(self.context.device.backend.target)?;
        unsafe { check("QnnGraph_addNode", (i.graph_addNode)(self.handle, op_config)) }
    }
    ...
    pub unsafe fn execute_raw(
        &self,
        inputs: *const sys::QnnTensor, n_in: u32,
        outputs: *mut sys::QnnTensor, n_out: u32,
    ) -> Result<()> {
        if !self.finalized {
            return Err(Error::Precondition {
                op: "Graph::execute_raw", msg: "graph not finalized".into(),
            });
        }
        let i = iface(self.context.device.backend.target)?;
        unsafe {
            check("QnnGraph_execute",
                  (i.graph_execute)(self.handle, inputs, n_in, outputs, n_out,
                                    std::ptr::null_mut(), std::ptr::null_mut()))
        }
    }

    #[inline] pub fn context(&self) -> &Arc<Context> { &self.context }
    #[inline] pub fn raw(&self) -> Qnn_GraphHandle_t { self.handle }
    #[inline] pub fn is_finalized(&self) -> bool { self.finalized }
}

// ── availability ────────────────────────────────────────────────────────────

pub fn is_available(target: Target) -> bool { sys::is_available(target) }
