...
fn probe_all() -> u8 {
    let mut m = 0u8;
    if drv::is_available(Target::Htp) {
        m |= 1 << 0;
    }
    if drv::is_available(Target::Gpu) {
        m |= 1 << 1;
    }
    if drv::is_available(Target::Cpu) {
        m |= 1 << 2;
    }
    if drv::is_available(Target::Dsp) {
        m |= 1 << 3;
    }
    m
}
...
impl Backend for QnnBackend {
    ...
    fn enumerate(&self) -> Result<Vec<DeviceDescriptor>> {
        ...
        for (ord, target) in self.available_targets().into_iter().enumerate() {
            ...
            out.push(DeviceDescriptor {
                id: DeviceId {
                    backend: BackendKind::QualcommNpu,
                    ordinal: ord,
                },
    ...
    fn capabilities(&self, device: u32) -> Result<CapabilityFlags> {
        let t = target_of(device).ok_or(ironaccelerator_core::Error::InvalidArgument(
            "unknown QNN ordinal",
        ))?;
